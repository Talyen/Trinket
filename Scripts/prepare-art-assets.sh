...
printf '%s\n' "# asset_name	source_sha256	encode_profile" >> "$source_hashes_temp"
trinket_asset_begin_state_lookup "$source_hashes_file"

combatant_ids_temp=$(mktemp)
ability_ids_temp=$(mktemp)
item_ids_temp=$(mktemp)
trinket_asset_extract_swift_quoted_ids "$combatant_ids_temp" \
  "$generated_dir/GameContentRoster.generated.swift" \
  "$generated_dir/GameContentEnemies.generated.swift"
trinket_asset_extract_swift_quoted_ids "$ability_ids_temp" \
  "$content_dir/AbilityCatalogBasic.swift" \
  "$content_dir/AbilityCatalogSkill.swift" \
  "$content_dir/AbilityCatalogUltimate.swift"
trinket_asset_extract_swift_quoted_ids "$item_ids_temp" \
  "$generated_dir/GameContentItemBases.generated.swift"

manifest_combatant_ids_temp=$(mktemp)
manifest_ability_ids_temp=$(mktemp)
manifest_item_ids_temp=$(mktemp)
awk -F'\t' '$1 == "combatant" {print $2}' "$manifest" | LC_ALL=C sort -u > "$manifest_combatant_ids_temp"
awk -F'\t' '$1 == "ability" {print $2}' "$manifest" | LC_ALL=C sort -u > "$manifest_ability_ids_temp"
awk -F'\t' '$1 == "item" {print $2}' "$manifest" | sed -e 's/-astral$//' -e 's/-basic$//' | LC_ALL=C sort -u > "$manifest_item_ids_temp"
...
check_manifest_ids_present "$manifest_combatant_ids_temp" "$combatant_ids_temp" \
  "Combatant art id" "is not present in the generated roster or enemy catalog"
check_manifest_ids_present "$manifest_ability_ids_temp" "$ability_ids_temp" \
  "Ability art id" "is not present in an authored ability catalog"
check_manifest_ids_present "$manifest_item_ids_temp" "$item_ids_temp" \
...
cleanup() {
  rm -f "$source_hashes_temp" "$contents_json_temp" "$combatants_temp" "$abilities_temp" "$items_temp" \
    ...
    "$manifest_combatant_ids_temp" "$manifest_ability_ids_temp" "$manifest_item_ids_temp" \
    "$prune_pairs_temp"
...
while IFS=$'\t' read -r kind id asset_name source_path focal_x focal_y || [[ -n "${kind:-}" ]]; do
  ...
  trinket_asset_assert_unique "$seen_ids_temp" "art id '$id' for kind '$kind'" "$kind:$id"
  if [[ ! "$asset_name" =~ ^[A-Za-z0-9_]+$ ]]; then
    echo "Asset name '$asset_name' should use only letters, numbers, and underscores." >&2
    exit 1
  fi
  trinket_asset_assert_unique "$seen_assets_temp" "art asset name" "$asset_name"
  ...
  printf '%s\t%s\t%s\n' "$asset_name" "$source_hash" "$encode_profile" >> "$source_hashes_temp"
...
trinket_asset_sort_state "$source_hashes_temp" "$source_hashes_file"
...
prune_pairs_temp=$(mktemp)
shopt -s nullglob
for dir in "$asset_catalog"/*/*.imageset "$asset_catalog"/*.imageset; do
  [[ -d "$dir" ]] || continue
  foldername=$(basename "$dir")
  name="${foldername%.imageset}"
  printf '%s\t%s\n' "$name" "$dir" >> "$prune_pairs_temp"
done
shopt -u nullglob
LC_ALL=C sort -t$'\t' -k1,1 -o "$prune_pairs_temp" "$prune_pairs_temp"
LC_ALL=C sort -u -o "$active_assets_temp" "$active_assets_temp"
while IFS=$'\t' read -r name dir; do
  [[ -n "${name:-}" && -n "${dir:-}" ]] || continue
  echo "Pruning orphaned asset: $(basename "$dir")"
  rm -rf "$dir"
done < <(join -t$'\t' -v 1 -1 1 -2 1 "$prune_pairs_temp" "$active_assets_temp")
rm -f "$prune_pairs_temp"
