...
final class KeywordTests: XCTestCase {
    func testAllKeywordsAreCovered() {
        let expected: Set<String> = ["Physical", "Burn", "Stun", "Block", "Armor", "Health", "Gold", "Holy", "Poison", "Bleed", "Leech", "Nature", "Freeze"]
    ...
    func testDamageTypeCategory() {
        ...
        for kw in types { XCTAssertEqual(kw.category, .damageType, "\(kw.rawValue) should be damageType") }
    }

    func testPreventionCategory() {
        let types: [Keyword] = [.stun, .freeze]
        for kw in types { XCTAssertEqual(kw.category, .prevention, "\(kw.rawValue) should be prevention") }
    }

    func testMitigationCategory() {
        let types: [Keyword] = [.block, .armor]
        for kw in types { XCTAssertEqual(kw.category, .mitigation, "\(kw.rawValue) should be mitigation") }
    }

    func testRestorationCategory() {
        let types: [Keyword] = [.health, .leech]
        for kw in types { XCTAssertEqual(kw.category, .restoration, "\(kw.rawValue) should be restoration") }
    }
    ...
    func testCategoryAllCases() {
        let expected: Set<String> = ["Damage Type", "Prevention", "Mitigation", "Restoration", "Resource"]
