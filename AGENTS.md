...
- Do not drop launch or imminent artwork pins, or switch first-screen art to on-demand `Image(name)`, to save memory. That working set is hitch prevention: decode before the tap, pin so deferred catalog warmup cannot evict it. `NSCache` alone is not enough. Rationale: [`PreparedArtworkCache`](Packages/TrinketFeatureSupport/Sources/TrinketFeatureSupport/PreparedArtworkCache.swift) and [PerformanceInvestigationPlaybook](Docs/Platform/PerformanceInvestigationPlaybook.md).
