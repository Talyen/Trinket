...
- New player flows need stable accessibility identifiers suitable for smoke tests. Add or extend UI smoke only when the keep/drop rubric in `Docs/Platform/Testing.md` applies. Preserve native semantics, label custom interactive controls, and use shared DesignSystem/motion handling for accessibility settings per PD-007.
