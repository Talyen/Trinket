...
|------|------|------------------------------------|
| 2026-09-11 | Agent discovery and broad script routing | [Authored search](../Scripts/agent-search.py) bounds discovery with explicit omissions; [context routing](../Docs/AgentContext/README.md) selects concern contracts; [verification](../Docs/Platform/Verification.md) routes Markdown before script/manifest roots and scopes leaf regressions while shared inputs retain full coverage. [Measurements](evals/context-efficiency.md) distinguish context savings from unmeasured task-token savings. |
