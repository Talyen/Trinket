...
struct EffectSummaryBuilderTests {
    ...
    @Test func shieldSummary() throws {
        ...
        let summaries = EffectSummaryBuilder.build(for: effects)
        try #expect(summaries.first?.text == "Block: 5 buffer, 6 ticks left.")
    }
    ...
    @Test func mitigationSummary() throws {
        ...
        let summaries = EffectSummaryBuilder.build(for: effects)
        try #expect(summaries.first?.text == "Armor: 25% mitigation, 3 ticks left.")
    }
    ...
    @Test func leechSummary() throws {
        ...
        let summaries = EffectSummaryBuilder.build(for: effects)
        try #expect(summaries.first?.text == "Leech: 10% leech, 6 ticks left.")
    }
