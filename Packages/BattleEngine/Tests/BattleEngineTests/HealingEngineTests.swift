...
struct HealingEngineTests {
    ...
    func `resolve heal emits events only for instant heal policy`(emitsEvent: Bool) throws {
    ...
    @Test func `leech from damage heals and sets leeched flag`() throws {
        var context = makeContext(seed: BattleTestFixtures.deterministicNonCriticalSeed)
    ...
    @Test func `keyword damage leech heals at ability percent`() throws {
    ...
    @Test func `ability leech heals half of damage dealt`() throws {
        var context = makeContext(seed: BattleTestFixtures.deterministicNonCriticalSeed)
        context.roster.mutateRuntime(for: context.roster.hero.combatant) { $0.currentHealth = 30 }
        let before = context.roster.hero.currentHealth
        let outcome = HealingEngine.leechFromDamage(
    ...
    @Test func `blood link routes overheal to companion and emits leech event`() throws {
        var context = BattleTestFixtures.makePipelineContext(
            heroModifiers: CombatModifierProfile(
                triggers: CombatTraitTriggers(
    ...
    @Test func `blood link into full health companion produces no leech outcome`() throws {
        var context = BattleTestFixtures.makePipelineContext(
            heroModifiers: CombatModifierProfile(
                triggers: CombatTraitTriggers(
                    healing: HealingTriggers(leechOverhealTransfersToCompanion: true),
    ...
    @Test func `leech from damage does not revive defeated source`() throws {
        var context = makeContext(seed: BattleTestFixtures.deterministicNonCriticalSeed)
    ...
    @Test func `resolve heal ignores defeated target`() throws {
        var context = makeContext(seed: BattleTestFixtures.deterministicNonCriticalSeed)
    ...
    @Test func `heal from one HP while deaths door active`() throws {
        var context = makeContext(seed: BattleTestFixtures.deterministicNonCriticalSeed)
    ...
    @Test func `heal does not remove deaths door effect`() throws {
        var context = makeContext(seed: BattleTestFixtures.deterministicNonCriticalSeed)
        let hero = context.roster.hero.combatant
    ...
    @Test func `instant heal can critical with wisdom`() throws {
    ...
    @Test func `leech heal can critical with wisdom`() throws {
        var context = BattleTestFixtures.makePipelineContext(
    ...
    @Test func `silent heals do not roll critical`() throws {
    ...
    @Test func `heal over time on heal arms ho T but tick does not rearm`() throws {
