...
extension BattleState {
    func talentEffectSummaries(of combatant: Combatant) -> [EffectSummary] {
        ...
        return pending + timedEffectSummaries(runtime.talents.timed)
            + preparedEffectSummaries(heroTalents.history[combatant.id])
    }

    private func timedEffectSummaries(_ timed: CombatantTalentState.Timed) -> [EffectSummary] {
        var summaries: [EffectSummary] = []
        if timed.dodge.amount > 0, timed.dodge.expiresAtTurn == 0 || turnCount < timed.dodge.expiresAtTurn {
            summaries.append(EffectSummary(keyword: .dodge, text: "Dodge Up: +\(Int((timed.dodge.amount * 100).rounded()))% Dodge chance."))
        }
...
private extension CombatantTalentState.Pending {
    func effectSummaries(criticalAppliesToParty: Bool) -> [EffectSummary] {
