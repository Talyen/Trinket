...
struct ArtworkViewportPrewarmTests {
    @Test func emptyVisibleIDsPrefetchesInitialWindow() {
    ...
    @Test func visibleWindowIncludesVisiblePlusThreeRowsAhead() {
        let items = (0 ..< 20).map { ViewportItem(id: "\($0)", thumbnail: "thumb\($0)") }
        let names = ArtworkViewportPrewarm.windowNames(
            orderedItems: items,
            visibleIDs: ["3", "4", "5"],
    ...
    @Test func windowClampsAtEndOfCollection() {
    ...
    @Test func deduplicatesThumbnailNames() {
        let items = [
    ...
    @Test func stringIDOverloadMatchesWindowLogic() {
        let items = (0 ..< 20).map { ViewportItem(id: "\($0)", thumbnail: "thumb\($0)") }
