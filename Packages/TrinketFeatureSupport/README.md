...
- **Shine System**: `Shine` is the single source for text and border shimmer (keyword, color, unique, corruption). `ProductCardShell` takes `Shine` directly; `ItemCard` derives it from rarity + astral keywords. `KeywordShine` / `KeywordShineBorder` are intentional presentation adapters, not a second shine owner.
- **Frame Pacing Diagnostics**: `FramePacingAnalyzer` and `FramePacingIntervalModifier` provide signpost instrumentation and refresh-normalized interval analytics for render delivery, stalls, and 1% low framerate. `FramePacingReport` is `Codable`; the UI-test transport preserves its supported schema compatibility.
