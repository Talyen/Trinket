...
Alternate destinations under Play are **Modes** (see `Docs/Design/AspectsAndModesPlan.md`). The first Mode is **Aspects**: affinity-themed floor climbs that reuse the same combat systems. The second Mode is **The Labyrinth**: a persistent infinite procedural delve with biome clusters and named modifiers (no umbrella affix noun in UI — titles only, like Item affixes); full spec in `Docs/Design/DelveModePlan.md`. Distinct from Aspects’ linear climbs. Player-facing Modes/Aspects/Labyrinth copy never says “Keyword”; Aspects use poetic titles (e.g. Cinder Spire for Burn).
