...
- **Agent isolation (`--isolate`)**: `handoff.sh --isolate` shares the simulator-slot environment from `Scripts/run-env.sh` (`Trinket Agent N` in Simulator.app, `.DerivedData/runs/agent-N/`, `TMPDIR`, `TRINKET_RUN_ID`). Human `run` / non-isolate tests use **Trinket Run**. Run-env self-clean reclaims non-empty Preview sims (shutdown Booted), enforces one Booted managed sim, and age-prunes bulky build artifacts. xcode-runner wall/idle watchdogs kill host xcodebuild trees only (no `simctl`).
...
- **Push gates**: Land on `main` by direct push; do not open pull requests. Use `./Scripts/agent-push-gate.sh` for post-commit generate/assert checks (does not run style/compile). Do not require `tests / CI OK` as a GitHub push gate (chicken-and-egg with `ci.yml`).
