...
- Deleted feature? Delete its test file, its feature folder, and its entry in `project.yml` (then run `./Scripts/generate.sh`)
- Deleted model type? Remove its coded conformance, its test fixtures, and any serialization glue

### Generated code drift

- After editing `ContentManifest/`, `ArtManifest/`, `MusicManifest/`, or `SoundManifest/`, run `./Scripts/generate.sh` to regenerate catalogs
...
- `./Scripts/assert-generated-output.sh` (CI gate) catches drift before commit — do not bypass
...
- No test files that only contain `import XCTest` with no test methods and no test class
