...
- **Options/preferences:** `OptionsStore` persists appearance, volumes, and haptics via `AppStorage`-compatible keys on a local `UserDefaults` suite — intentionally **not** part of `PlayerSave` / CloudKit. Session keys (tab/battle restoration) remain on the shell session store. Collection discovery acknowledgment (`viewedCombatantIDs` / `viewedItemIDs`) lives on `PlayerSave.collectionAttention`, not the shell session.
...
- **Collection attention:** `PlayerCollectionAttentionState` tracks which unlocked combatants and owned items the player has opened in Collection detail sheets. Tab badges and per-card NEW markers derive from unlocks/inventory minus these viewed sets. Starters are always treated as viewed.
